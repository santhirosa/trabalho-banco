...
int recebe_arq_principal(t_cliente*);
...
//Subrotinas auxiliares para limpeza de tela//
void cls(HANDLE);
void pausa(void);

//Subrotinas auxiliares para trabalho com datas//
...
int realiza_cadastro(cliente,qnt_anterior)
t_cliente *cliente; int qnt_anterior;
{
    int i=qnt_anterior-1;
    srand(time(NULL));
...
int recebe_arq_principal(cliente)
t_cliente *cliente;
{
    FILE *fp;
    int i;
...
int main()
{
    ...
    int retorno=0, continuar=1, cont=0, qnt_anteriror_registrados;
    int continuar_1=0;
    ...
    qnt_anteriror_registrados=recebe_arq_principal(lista);
    while(continuar)
    {
        ...
        if(retorno == 2)
        {
            cont=realiza_cadastro(lista,qnt_anteriror_registrados);
        }
        if(retorno == 3) exit(-1);
